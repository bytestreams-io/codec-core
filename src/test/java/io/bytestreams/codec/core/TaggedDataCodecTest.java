...
class TaggedDataCodecTest {

  @Test
  void encode_decode_single_tag() throws IOException {
    Codec<TaggedData<String>> codec =
        Codecs.<String>tagged(Codecs.ascii(4)).tag("code", Codecs.uint16()).build();

    TaggedData<String> original = new TaggedData<>();
    original.add("code", 42);

    ByteArrayOutputStream output = new ByteArrayOutputStream();
    codec.encode(original, output);

    TaggedData<String> decoded = codec.decode(new ByteArrayInputStream(output.toByteArray()));
